...
DEFAULT_SETTINGS = {
    "ai": "chatgpt",           # chatgpt | claude | copy
...
def get_transcription(id_: int, full: bool = True) -> dict | None:
    columns = f"{SUMMARY_COLUMNS}, segments, file_name" if full else SUMMARY_COLUMNS
...
def create_client(name: str, phone: str = "", notes: str = "") -> int:
    return _execute(
        "INSERT INTO clients (name, phone, notes, created_at) VALUES (?, ?, ?, ?)",
        (name, phone, notes, time.time()),
    ).lastrowid
